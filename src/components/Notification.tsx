...
const Notification = ({ type, message, description, txid, onHide }) => {
  ...
  return (
    <div
    className={`max-w-xs w-full bg-bkg-0 shadow-lg rounded-md mt-0 pointer-events-auto ring-1 ring-black ring-opacity-5 p-20 mx-0 mb-0 overflow-visible`}
    >
      <div className={`p-4`}>
        <div className={`flex items-center`}>
          <div className={`flex-shrink-0`}>
            {type === 'success' ? (
              <CheckCircleIcon className={`h-12 w-12 mr-0 text-bold`} />
            ) : null}
